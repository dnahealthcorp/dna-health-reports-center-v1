...
const Layout = ({
  ...
  useEffect(() => {
    // Only redirect if auth check is complete and not on auth pages
    if (!isLoading && !user && 
  ...
  // If no user and not on auth pages, show a placeholder to prevent flash
  ...
  // On auth pages, don't render the layout if there's no user
  if (!user && (
      location.pathname === "/login" || 
      location.pathname === "/set-password" || 
      location.pathname.startsWith("/auth/")
